...
by `r Biocpkg("treeio")` [@wang_treeio_2020] can be converted to tidy
...
The `triple_tree` object showed above contains analysis results obtained from `r pkg_beast`
and `r pkg_codeml`, and evolutionary
traits from external sources. All these information can be used to annotate the
...
In addition, with the `r CRANpkg("tidytree")` package, manipulating tree is more easier using the [tidy data principles](https://www.jstatsoft.org/article/view/v059i10) and 
...
used directly in tree visualization (*e.g.* [coloring tree based on grouping information](#group-taxa-vis))
...
A phylogenetic tree can be rerooted with a specified `outgroup`. The `r CRANpkg("ape")` package implements a `root()` method to reroot a tree stored in a `phylo` object, while the `r Biocpkg("treeio")` package provides the `root()` method for `treedata` object. This method is designed to re-root a phylogenetic tree with associated data with respect to the specified `outgroup` or at the specified `node` based on the `root()` implemented in the `r CRANpkg("ape")` package.


We linked external data to a tree and stored all the information in a `treedata` object, `trda`. 

```{r}
tree <- read.tree("data/tree_boots.nwk")
# the tip label information
tip.da <- read.csv("data/tip_data.csv")
# the node label information
node.da <- read.csv("data/inode_data.csv")
# These associated dataset can be added to the treedata using left_join or full_join 
trda <- tree %>% 
        left_join(tip.da, by=c("label" = "Newick_label")) %>% 
        left_join(node.da, by=c("label" = "newick_label"))
trda
```


Then we can reroot the tree with associated data mapping to the branches and nodes correctly as demonstrated in Figure \@ref(fig:reroot). The figure was visualized using `r Biocpkg("ggtree")` (see also Chapter [4](#chapter4) and [5](#chapter5)).
...
Sometimes a tree can be large and difficult to look at only the portions of interest. The `tree_subset()` function was created in the `r Biocpkg("treeio")` package [@wang_treeio_2020] to extract a subset of the tree portion while still maintaining the structure of the tree portion. The `beast_tree` in Figure \@ref(fig:subsetTip)A is slightly crowded. Obviously, we can make the figure taller to allow more space for the labels (similar to use "Expansion" slider in `FigTree`) or we can make the text smaller. However, these solutions are not always applicable when you have a lot of tips (*e.g.* hundreds or thousands of tips). In particular, when you are only interested in the portion of the tree around a particular tip, you certainly don't want to explore a large tree to find the centain species you are interested in.
...
The `r Biocpkg("treeio")` package allows us to import diverse phylogeny associated data into R. However, phylogenetic tree is stored in a way to facilitate computational processing which is not human friendly and needs the expertise to manipulate and explore tree data. The `r CRANpkg("tidytree")` package provides tidy interface for exploring tree data, while `r Biocpkg("ggtree")` provides a set of utilities to visualize and explore tree data using the grammar of graphics. This full suite of packages makes it easy for ordinary users to interact with tree data and allow us to integrate phylogeny associated data from different sources (*e.g.* experimental result or analysis finding), which creates the possibilities of integrative and comparative study. Moreover, this package suite brings phylogenetic analysis into the tidyverse and certainly takes us to the next level of processing phylogenetic data.
