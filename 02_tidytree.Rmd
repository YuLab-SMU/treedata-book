...
The phylogenetic tree can be rerooted with specified `outgroup`. For the `phylo` class, the `root()` of CRANpkg("ape") can be used to reroot the tree. The Biocpkg("treeio") also implemented `root()` method for `treedata` object. This method is designed to re-root a phylogenetic tree with associated data via respect to the specified `outgroup` or at the specified `node` based on the `root()` of CRANpkg("ape").
...
The `outgroup` parameter represent the specific new `outgroup`, it can be character or node number, and if it is a single one value, meaning using the node below this tip as the new root, if it has multiple values, meaning the most recent common of the values will be used as the new root. Note that, if the node labels should be as the edge labels, the `edgelabel` should be set to `TRUE` to return the correct relationship between `node` and `associated data`, this feature inherits the `root()` of CRANpkg("ape"), which was introduced according to the review [@reroot_review].
