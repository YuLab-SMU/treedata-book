...
The `r Biocpkg("ggtree")` package supports both `hclust` and `dendrogram` objects. Users can use `ggtree(object)` to display its tree structure, and user other layers and utilities to customize the graph and of course add annotation to the tree.
...
```{r ggtreehclust, fig.width=8, fig.height=5, fig.cap="(ref:ggtreehclustcap)", fig.scap="(ref:ggtreehclustscap)"}
clus <- cutree(hc, 4)
...
ggtree(hc, linetype='dashed', color = "#487AA1") %<+% d + 
  layout_dendrogram() + 
  geom_tippoint(size=5, shape=21, aes(fill=factor(cyl), x=x+.5)) + 
  geom_tiplab(aes(label=cyl), size=3, hjust=.5) +
  geom_tiplab(aes(color = member), angle=90, hjust=1, offset=-5) + 
