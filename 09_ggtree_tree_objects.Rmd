# ggtree for other tree-like objects {#chapter10}
...
Both `hclust` and `dendrogram` objects can be converted to `phylo` object via `ape::as.phylo` (for `hclust`) and `phylogram::as.phylo` (for `dendrogram`).

After converting the object to `phylo`, we can use `ggtree` to display its tree structure, customize the graph and of course add annotation to the tree.
