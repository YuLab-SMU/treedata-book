...
#The rownames (usually is OTUs or other features ) of the taxa will be as the tip labels
#if include.rownames = TRUE
tree <- as.treedata(taxa, include.rownames=TRUE)
# Or extract the taxa tree (treedata) with mp_extract_tree, because the 
# taxonomy information is stored as treedata in MPSE class (kostic2012crc).
...
Another way to print ASCII-art of phylogeny is to use the `ascii()` device defined in [devout](https://github.com/coolbutuseless/devout) package. Here is an example:
