...
The `r Biocpkg("ggtree")` [@yu_ggtree:_2017] is designed for a more general purpose or a specific type of tree visualization and annotation. It supports grammar of graphics\index{grammar of graphics} implemented in `r CRANpkg("ggplot2")` and users can freely visualize/annotate a tree by combining several annotation layers. 

(ref:ggtreeNHXscap) Annotating tree using grammar of graphics.

(ref:ggtreeNHXcap) **Annotating tree using grammar of graphics.** The NHX tree was annotated using grammar of graphic syntax by combining different layers using `+` operator. Species information was labelled on the middle of the branches, Duplication events were shown on the most recent common ancestor and clade bootstrap values were displayed near to it.
...
The `geom_cladelab()` layer accepts a selected internal node number and label corresponding clade automatically (Figure \@ref(fig:cladelabel)A). To get the internal node number, please refer to [Chapter 2](#accesor-tidytree). 
...
`geom_cladelab` also supports unrooted tree layouts (Figure \@ref(fig:striplabel)A).
...
The `geom_cladelab` layer also supports using aesthetic mapping to automatically annotate the clade with bar and text label (or image)

```{r eval=F}
dat <- data.frame(node = c(45, 34), name = c("test label", "another clade"))
# The node and label is required when geom="text" or geom="label" or geom="shadowtext".
p + geom_cladelab(data = dat, mapping = aes(node = node, label = name, color = name), fontsize = 3)
```

In addition, it also allow users to use the image or phylopic to annotate the clades.
...
p + geom_cladelab(data = dt, mapping = aes(node = node, label = name, image = image), geom = "phylopic", imagecolor = "black", offset=1, offset.text=0.5)
# The color or size of image also can be mapped.
p + geom_cladelab(data = dt, mapping = aes(node = node, label = name, image = image, color = name), geom = "phylopic", offset = 1, offset.text=0.5)
```

(ref:cladelabaesscap) Labeling clades using aesthetic mapping.
(ref:cladelabaescap) **Labeling clades using aesthetic mapping.** geom_cladelab also allows users to use aesthetic mapping to annotate the clades (A). It also supports using images or phylopic to annotate clades (B). The character (such as color or size) of text or image can also be mapped (C).
...
plot_grid(f5, f6, f7, ncol=3, labels=LETTERS[1:3]) 
```

`geom_cladelab` is designed for labeling Monophyletic (Clade) while there are related taxa that are not form a clade. `ggtree` provides `geom_strip` to add a strip/bar to indicate the association with optional label for Polyphyletic or Paraphyletic (Figure \@ref(fig:striplabel)B).
...
(ref:striplabelcap) **Labeling associated taxa.** `geom_cladelab` is designed for labeling Monophyletic and it also supports unrooted layout (A). `geom_strip` is designed for labeling associated taxa (Monophyletic, Polyphyletic or Paraphyletic) (B).
...
`ggtree` implements _`geom_hilight`_ layer, that accepts an internal node number and add a layer of a rectangle to highlight the selected clade Figure (\@ref(fig:hilight)) ^[If you want to plot the tree above the highlighting area, visit [FAQ](#faq-under-the-tree) for details.].
...
Ancestral sequences inferred by *HyPhy* can be parsed using [*treeio*](#parsing-hyphy-output), whereas the substitutions along each tree branch were automatically computed and stored inside the phylogenetic tree object (*i.e.*, `S4` object). The *ggtree* can utilize this information in the object to annotate the tree, as demonstrated in Figure \@ref(fig:hyphy).
...
(ref:codemlscap) Annotating tree with amino acid substitution and *d~N~/d~S~* inferred by *CodeML*.

(ref:codemlcap) **Annotating tree with amino acid substitution and *d~N~/d~S~* inferred by *CodeML*.** Branches were rescaled and colored by *d~N~/d~S~* values and amino acid substitutions were displayed on the middle of branches.
...
Not only all the tree data parsed by `r Biocpkg("treeio")` can be used to visualize and annotate phylogenetic tree using `ggtree`, but also other tree and tree-like objects defined in R community are supported. The `r Biocpkg("ggtree")` plays a unique role in R ecosystem to facilitate phylogenetic analysis and it can be easily integrated into other packages and pipelines. For more details, please refer to [chapter 9](#chapter9). In addition to direct support of tree objects, `r Biocpkg("ggtree")` also allows users to plot tree with different types of external data (see also [chapter 7](#chapter7) and [@yu_two_2018]).


## Summary {#summary5}


The `r Biocpkg("ggtree")` package implements grammar of graphics for annotating phylogenetic trees. Users can use ggplot2 syntax to combine different annotation layers to produce complex tree annotation. If you are familiar with `r CRANpkg("ggplot2")`, tree annotation with a high level of customization can be intuitive and flexible using `r Biocpkg("ggtree")`.
