...
As this is a very common issue, we implemented a `read.newick()` function in the `r Biocpkg("treeio")` package to allow parsing internal node labels as support values. So that it can be more easy to disaply bootstrap values using the following code:
