...
Some of the functions, *e.g.* `viewClade`, work with clade and accept a parameter of an internal node number. To get the internal node number, users can use `MRCA()` function (as in Figure \@ref(fig:viewClade)) by providing two taxa names. The function will return the node number of input taxa's most recent common ancestor (MRCA). It works with tree and graphic (*i.e.* `ggtree()` output) object. `r CRANpkg("tidytree")` also provide `MRCA` function to extract information of MRCA node (see details in [session 2.1.3](#accesor-tidytree)).
