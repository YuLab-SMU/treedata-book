...
(ref:tdfilterscap) An example of filtering ggtree plot data in geom layers.

(ref:tdfiltercap) **An example of filtering ggtree plot data in geom layers.**
...
(ref:tdunnestscap) Flatten ggtree plot data.

(ref:tdunnestcap) **Flatten ggtree plot data.** (A) list-columns can be flattened by `td_unnest()`. (B) Different tree data utilites can be combined to work together (e.g. filter data by `td_filter()` and then flatten it by `td_unnest()`. 

```{r tdUnnest,fig.cap="(ref:tdunnestcap)", fig.scap="(ref:tdunnestscap)", fig.width=8, fig.height = 5}
p <- ggtree(tr) %<+% d2
p2 <- p + 
