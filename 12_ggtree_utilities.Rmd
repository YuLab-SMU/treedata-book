...
(ref:hexpandcap) **Expanding plot limits by a fraction of the x or y range.** expand x limit at right-hand side by default (A). expand x limit for left-hand side when direction = -1 and expand y limit at the upper side (B). 
...
To create [composite plots](#composite_plot), users need to re-order their data manually before create tree-associated graphs. The order of their data should be consistent with the tip order presented in ggtree plot. For this purpose, we provide the `get_taxa_name()` function to extract an ordered vector of tips based on the tree structure plotted by ggtree. 
