...
(ref:basicvizcap) **Basic tree visualization.** Default ggtree output with ladderized effect (A), non-variable setting (*e.g.* color, size, line type) (B), nonladderized tree (C), cladogram that only displays tree topology without branch length information (D).
...
**Cladogram.** To visualize a cladogram\index{cladogram} that is without branch length scaling and only display the tree structure,  `branch.length` is set to "none" and it works for all types of layouts (Figure \@ref(fig:layout)I-L).
