...
Users can use `phytools::bind.tip()` [@revell_phytools_2012] to attach a new tip to a tree. With `r CRANpkg("tidytree")`, it is easy to add annotation to differentiate newly introduce and original branches as demonstrated in Figure \@ref(fig:bindTip). 
...
```{r bindTip, fig.cap="(ref:bindTipcap)", fig.scap="(ref:bindTipscap)", fig.width=6, fig.height=4}
library(phytools)
library(tidytree)
library(ggtree)
...
set.seed(2019-11-18)
tr <- rtree(5)
tr2 <- bind.tip(tr, 'U', edge.length = 0.1, where = 7)
d <- as_tibble(tr2)
d$type <- "original"
d$type[d$label == 'U'] <- 'newly introduce'
tr3 <- as.treedata(d)
ggtree(tr3, aes(linetype=type)) + 
