...
To address this issue, `r Biocpkg("ggtree")` provides a `ylim_align()` function to reconcile y limits. 
...
The `ylim_align()` is not designed specific for `r Biocpkg("ggtree")`, it works fine with `ggplot` object. Another function `xlim_align()` that works for x limits is also provided. See [session 10.5](#axis_align) for more details.
