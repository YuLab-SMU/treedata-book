...
levels. The `r Biocpkg("treeio")` package [@wang_treeio_2020] implements `full_join` methods to
...
Companion functions to adjust [panel widths](#facet_widths) and [rename panel name](#facet_labeller) are described in [session 12.1](#facet-utils). Removing panel name is also possible and example was presented in Figure \@ref(fig:gggenes). We can also use `r CRANpkg("cowplot")` or `r Githubpkg("thomasp85", "patchwork")` to create composite plots as described in [session 7.6](#composite_plot). 
