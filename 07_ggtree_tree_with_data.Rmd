...
In addition to aligning graph to a tree using [geom_facet](#facet_plot) and special cases using [gheatmap](#gheatmap) and [msaplot](#msaplot), users can use `r CRANpkg("cowplot")` or `r Githubpkg("thomasp85", "patchwork")` to create composite plots.
...
+ the plotted data have different y-limits
+ the different plots have different amounts of expansion space 

To address these two issues, `r Biocpkg("ggtree")` provides a `ylim_ggtree` scale function to set y limits of a `ggplot` object based on the y limits of the input `ggtree` object. If the plot was flipped (such as `p1`), the `ylim_ggtree` will throw a message and apply the y limits to x-axis.
