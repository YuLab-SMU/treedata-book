...
To address these two issues, `r Biocpkg("ggtree")` provides a `align_y` scale function to set y limits of a `ggplot` object based on the y limits of the input `ggtree` object. If the plot was flipped (such as `p1`), the `align_y` will throw a message and apply the y limits to x-axis.
