...
```{r gheatmap, dev='svg', fig.process=svg2png, fig.width=8, fig.height=12, warning=FALSE, message=FALSE, echo=F,fig.cap="(ref:gheatmapcap)", fig.scap="(ref:gheatmapscap)", out.width='100%'}
beast_file <- system.file("examples/MCC_FluA_H3.tree", package="ggtree")
beast_tree <- read.beast(beast_file)
...
```{r msaplot, fig.width=14, fig.height=7, fig.process=svg2png, warning=FALSE, echo=F, fig.cap="(ref:msaplotcap)", fig.scap="(ref:msaplotscap)", out.width='100%'}
tree <- read.tree("data/tree.nwk")
p <- ggtree(tree) + geom_tiplab(size=3) + 
...
In addition to aligning graph to a tree using [geom_facet](#facet_plot) and special cases using [gheatmap](#gheatmap) and [msaplot](#msaplot), users can use `r CRANpkg("cowplot")`, `r CRANpkg("patchwork")`, `r CRANpkg("gtable")`^[<https://github.com/YuLab-SMU/ggtree/issues/313>] or other packages to create composite plots. However, extra efforts needs to be done to make sure all the plots are aligned properly. The [get_taxa_name()](#tiporder) function is quite useful for users to re-order their data based on the tree structure. To remove this obstacle, we created an R package `r CRANpkg("aplot")` that can re-order the internal data of 'ggplot' object and create composite plots that align properly with a tree.
...
(ref:compositecap) **Example of aligning tree with data side by side to create composite plot**. `cowplot` aligns subplots (A), while `aplot` do extra works to make sure that tree associated subplots are properly ordered according to the tree structure (B). Note: The 'A' category in the bar plot that is not matched with the tree was removed.

```{r composite, dev= "svg", echo = FALSE, fig.width=12, fig.height=10, fig.cap="(ref:compositecap)", fig.scap="(ref:compositescap)", out.width='100%'}
