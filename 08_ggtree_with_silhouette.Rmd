...
Although `R`'s graphical devices don't support `r icons::fontawesome("apple")` `AppleColorEmoji` font on MacOS, it's still possible to use it. We can export the `plot` to a `svg` file and render it in `r icons::icon_style(icons::fontawesome("safari"), fill ='steelblue')``Safari`.
...
(ref:applemojicap) **Use Apple Color Emoji in `ggtree`.** The background is an image of a *opisthocomus* from Phylopic database and the tip labels were parsed as emojis using `AppleColorEmoji` font. 
