...
Although `R`'s graphical devices don't support `r icons::fontawesome("apple")` `AppleColorEmoji` font, it's still possible to use it. We can export the `plot` to `svg` file and render it in `r icons::icon_style(icons::fontawesome("safari"), fill ='steelblue')``Safari`.
