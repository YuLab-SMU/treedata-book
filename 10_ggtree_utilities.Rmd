...
To address these two issues, `r Biocpkg("ggtree")` provides `xlim_align()` and `ylim_align()` functions to set x or y limits. It use input `limits` to set axis limits that is similar to `xlim()` and `ylim()` (Figure \@ref(fig:axisAlign)B). If `limits = NULL` (by default), the `xlim_align()` and `ylim_align()` functions will calculate axis limits from input `ggplot` object. So that we can easily set limits of a `ggplot` object based on another `ggplot` object to uniformize their limits (Figure \@ref(fig:axisAlign)C). 

```r
pp2 <- lapply(pp, function(p) p + xlim_align(limits=c(3, 11)))
pp3 <- lapply(pp, function(p) p + xlim_align(p1))
...
The example of using `ylim_align()` to reconcile y axis is described in [session 7.5](#composite_plot). 
