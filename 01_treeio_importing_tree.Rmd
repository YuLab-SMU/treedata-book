...
### Converting other tree object to `treedata` object {#as-treedata}

`r Biocpkg("treeio")` provides several `as.treedata` methods to convert other tree objects, such as `phylo4d` and `pml`, to `treedata` object, so that they can be easily manipulated and visualized using `r Biocpkg("treeio")` and `r Biocpkg("ggtree")`.


The `pml` object defined in `r CRANpkg("phangorn")` package computes the likelihood of a phylogenetic tree given a sequence alignment and a model and the `optim.pml` optimizes different model parameters. It can be converted to a `treedata` object and amino acid substitution (ancestral sequence estimated by `pml`) can be visualized using `r Biocpkg("ggtree")` as demonstrated in Figure \@ref(fig:pmlTree).
