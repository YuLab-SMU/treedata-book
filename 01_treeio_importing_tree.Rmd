...
*BEAST*\index{BEAST} [@bouckaert_beast_2014] output is based on NEXUS and it also introduces square brackets in the tree block to store evolutionary evidence inferred by *BEAST*. Inside brackets, curly braces may also be incorporated if feature values have a length more than 1 (*e.g.*, HPD or range of substitution rate). These brackets are placed between node and branch length (*i.e.*, after label if exists and before colon). Bracket is not defined in Newick format and is a reserve character for NEXUS comment. So these information will be ignored for standard NEXUS parsers. 
...
*BEAST* output can contain many different evolutionary inferences, depending on the analysis models defined in *BEAUTi* for running. For example in molecular clock analysis, it contains `rate`, `length`, `height`, `posterior` and corresponding HPD and range for uncertainty estimation. `Rate` is the estimated evolutionary rate of the branch. `Length` is the length of the branch in years. `Height` is the time from node to root while `posterior` is the Bayesian clade credibility value. The above example is the output tree of clock analysis and should contain these inferences. To save space, only the `length` estimation was shown above. Besides, MEGA [@kumar_mega7_2016] also supports exporting tree in BEAST compatible Nexus format (see [session 1.3.2](#mega)). 
...
[RevBayes](https://revbayes.github.io/intro.html) *etc*. This is made possible with the several parser functions developed in *treeio* (Table \@ref(tab:treeio-function)) [@wang_treeio_2020].
...
by
[EPA](http://sco.h-its.org/exelixis/web/software/epa/index.html) [@berger_performance_2011]
...
phylogenetic tree using `r Biocpkg("ggtree")`
...
using `r Biocpkg("ggtree")` package [@yu_ggtree:_2017] (described in Chapter [4](#chapter4) and [5](#chapter5)). 
...
knitr::include_graphics("img/treeio-diagram.png")
```
...
[MEGA](https://www.megasoftware.net/) [@kumar_mega7_2016] supports exporting trees in three distinct formats: Newick, tabular and Nexus. The Newick file can be parsed using the `read.tree` or `read.newick` functions. MEGA Nexus file is similar to BEAST Nexus and `r Biocpkg("treeio")` [@wang_treeio_2020] provides `read.mega` function to parse the tree.
...
tree nodes. [treeio](https://bioconductor.org/packages/treeio/) [@wang_treeio_2020] will automatically determine the substitutions between the
...
pressure and estimated *d~N~/d~S~*, *d~N~* and *d~S~*. This information are
...
In the previous session, we separately parsed *rst* and *mlc* files. However, they
...
[RAxML](http://evomics.org/learning/phylogenetics/raxml/) bootstrapping analysis
...
associated data inferred by PHYLDOG (Figure \@ref(fig:beastFigtree)A).
...
the right hand side of the tree using `r Biocpkg("ggtree")` (see also the [`msaplot` function](#msaplot) and Basic Protocol 5 of [@yu_cp_2020]).
...
[EPA](http://sco.h-its.org/exelixis/web/software/epa/index.html)
[@berger_performance_2011] and [PPLACER](http://matsen.fhcrc.org/pplacer/)
...
this [treeio](https://bioconductor.org/packages/treeio/) package [@wang_treeio_2020] to support tree
...
(ref:pmlTreecap) **Converting `pml` object to `treedata` object.** 
